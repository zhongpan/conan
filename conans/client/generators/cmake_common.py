...
function(conan_package_library_targets libraries package_libdir libraries_abs_path deps build_type package_name)
    foreach(_LIBRARY_NAME ${libraries})
        unset(CONAN_FOUND_LIBRARY CACHE)
        if(NOT package_libdir MATCHES ".conan")
            if(MSVC)
                if(_LIBRARY_NAME MATCHES "static_")
                    string(REPLACE "static_" "" _LIBRARY_NAME "${_LIBRARY_NAME}")
                    set(CONAN_FOUND_LIBRARY ${package_libdir}/${_LIBRARY_NAME}.lib)
                else()
                    set(CONAN_FOUND_LIBRARY ${package_libdir}/${_LIBRARY_NAME}.lib)
                endif()
            else()
                if(_LIBRARY_NAME MATCHES "static_")
                    string(REPLACE "static_" "" _LIBRARY_NAME "${_LIBRARY_NAME}")
                    set(CONAN_FOUND_LIBRARY ${package_libdir}/lib${_LIBRARY_NAME}.a)
                else()
                    set(CONAN_FOUND_LIBRARY ${package_libdir}/lib${_LIBRARY_NAME}.so)
                endif()
            endif()
        else()
            find_library(CONAN_FOUND_LIBRARY NAME ${_LIBRARY_NAME} PATHS ${package_libdir}
        ...
        if(CONAN_FOUND_LIBRARY)
            #message(STATUS "Library ${_LIBRARY_NAME} found ${CONAN_FOUND_LIBRARY}")
            if(_LIBRARY_NAME MATCHES "_stlp")
                string(REGEX REPLACE "_stlpd|_stlp" "" _LIBRARY_NAME ${_LIBRARY_NAME})
            endif()
            set(_LIB_NAME CONAN_LIB::${package_name}:${_LIBRARY_NAME}${build_type})
...
macro(conan_target_link_libraries target)
    if(${ARGC} GREATER 1)
        set(_link_targets ${ARGN})
		if(CONAN_TARGETS)
			list(FILTER _link_targets EXCLUDE REGEX "CONAN_LIB::")
			target_link_libraries(${target} ${_link_targets})
			set(_link_targets ${ARGN})
		else()
			list(FILTER _link_targets EXCLUDE REGEX "CONAN_PKG::|CONAN_LIB::")
			target_link_libraries(${target} ${_link_targets})
		endif()
    elseif(CONAN_TARGETS)
        target_link_libraries(${target} ${CONAN_TARGETS})
        set(_link_targets ${CONAN_TARGETS})
    else()
        ...
        endforeach()
        if(CONAN_LIBS)
		    string(REPLACE " " ";" _link_targets ${CONAN_LIBS})
		endif()
    endif()
	set(_dep_lib_targets "")
    foreach(_link_target ${_link_targets})        
        if(${_link_target} MATCHES "CONAN_PKG::")
            if(NOT TARGET ${_link_target})
                message(FATAL_ERROR "target not exist:${_link_target}")
            endif()
            get_target_property(_pkg_libs ${_link_target} INTERFACE_LINK_LIBRARIES)
            foreach(_pkg_lib ${_pkg_libs})
                if(${_pkg_lib} MATCHES "CONAN_LIB::")
                    string(REGEX REPLACE "CONAN_LIB::.+:" ""  _pkg_lib ${_pkg_lib})
                    if(TARGET ${_pkg_lib})
                        set(_dep_lib_targets ${_dep_lib_targets} ${_pkg_lib})  
                    endif()
                endif()
            endforeach()
        elseif(${_link_target} MATCHES "CONAN_LIB::")
            string(REGEX MATCH "^CONAN_LIB::(.+):(.+(,.+)*)$" outvar ${_link_target})
			set(_pkg_name ${CMAKE_MATCH_1})
			set(_lib_name ${CMAKE_MATCH_2})
            if(NOT _pkg_name OR NOT _lib_name)
                message(FATAL_ERROR "link library format wrong:${_link_target}")
            endif()
			string(REPLACE "," ";" _lib_name ${_lib_name})
			set(_pkg_target CONAN_PKG::${_pkg_name})
            if(NOT TARGET ${_pkg_target})
                message(FATAL_ERROR "pkg name wrong:${_link_target}")
            endif()
			string(TOUPPER CONAN_INCLUDE_DIRS_${_pkg_name} INC_DIRS_VAR)
			set(INC_DIRS ${${INC_DIRS_VAR}})
            if(INC_DIRS)
                get_target_property(INC_DIRS_OLD ${target} INCLUDE_DIRECTORIES)
                if(INC_DIRS_OLD)
                    set(INC_DIRS ${INC_DIRS} ${INC_DIRS_OLD})
                endif()
                list(REMOVE_DUPLICATES INC_DIRS)
                set_target_properties(${target} PROPERTIES INCLUDE_DIRECTORIES "${INC_DIRS}")
            endif()
			get_target_property(_pkg_libs ${_pkg_target} INTERFACE_LINK_LIBRARIES)
			list(FILTER _pkg_libs INCLUDE REGEX "CONAN_LIB::")
			list(TRANSFORM _pkg_libs REPLACE "CONAN_LIB::${_pkg_name}:" "")
			set(_result "")
			foreach(_pkg_lib ${_lib_name})
				if(${_pkg_lib} MATCHES "^-.+")
					if(NOT _exclude_result)
						set(_exclude_result ${_pkg_libs})
					endif()
					string(SUBSTRING ${_pkg_lib} 1 -1 _exclude_re)
					list(FILTER _exclude_result EXCLUDE REGEX ${_exclude_re})
				elseif(${_pkg_lib} MATCHES "^\\\\+.+")					
					string(SUBSTRING ${_pkg_lib} 1 -1 _include_re)
					set(_include_result ${_pkg_libs})
					list(FILTER _include_result INCLUDE REGEX ${_include_re})
					set(_result ${_result} ${_include_result})
				else()
					set(_lib_target CONAN_LIB::${_pkg_name}:${_pkg_lib})
					if(NOT TARGET ${_lib_target})
						message(FATAL_ERROR "target not exist:${_lib_target}")
					endif()
					set(_result ${_result} ${_pkg_lib})
				endif()
			endforeach()
			if(_exclude_result)
				set(_result ${_result} ${_exclude_result})
				unset(_exclude_result)
			endif()
			list(REMOVE_DUPLICATES _result)
			foreach(_pkg_lib ${_result})
				target_link_libraries(${target} CONAN_LIB::${_pkg_name}:${_pkg_lib})
				if(TARGET ${_pkg_lib})
					set(_dep_lib_targets ${_dep_lib_targets} ${_pkg_lib})
				endif()			
			endforeach()
        else()
			string(REGEX REPLACE "_stlpd|_stlp" "" _link_target ${_link_target})
			if(TARGET ${_link_target})
				set(_dep_lib_targets ${_dep_lib_targets} ${_link_target})
			endif()
			set(_have_global_lib TRUE)
        endif()
    endforeach()
	if(CONAN_TARGETS AND _have_global_lib)
		include_directories(${CONAN_INCLUDE_DIRS})
		get_target_property(_compile_options_old ${target} COMPILE_OPTIONS)
		set(_compile_options_new ${CONAN_DEFINES})
		if(_compile_options_old)
			set(_compile_options_new ${_compile_options_new} ${_compile_options_old})
		endif()
		set_property(TARGET ${target} PROPERTY COMPILE_OPTIONS "${_compile_options_new}")
	endif()	
    if(_dep_lib_targets)
        message(STATUS "add_dependencies(${target},${_dep_lib_targets})")
        add_dependencies(${target} ${_dep_lib_targets})  
    endif()    
endmacro()
"""
